...
class PortfolioArchitect:
    ...
    def allocate(self,
        ...
        a_rev = float(np.interp(stress, sp, [0.10, 0.15, 0.15, 0.20]))
        a_val = float(np.interp(stress, sp, [0.20, 0.25, 0.75, 0.70]))
        a_qual = float(np.interp(stress, sp, [0.20, 0.50, 0.90, 1.20]))
        a_vol = float(np.interp(stress, sp, [0.15, 0.50, 1.0, 1.50]))
        a_liq = float(np.interp(stress, sp, [0.15, 0.10, 0.10, 0.20]))

        # Objective Function Knobs
        risk_aversion = float(np.interp(stress, sp, [45.0, 50.0, 100.0, 300.0]))
        turn_penalty = float(np.interp(stress, sp, [0.0, 10.0, 70.0, 100.0]))
        alpha_scale = float(np.interp(stress, sp, [0.60, 0.40, 0.3, 0.3]))
        smooth_lam = float(np.interp(stress, sp, [0.9, 0.5, 0.2, 0.2]))

        # Position constraints
        net_lo_eff = float(np.interp(stress, sp, [0.95, 0.86, 0.86, 0.86]))
        net_hi_eff = float(np.interp(stress, sp, [1.09, 1.00, 0.9, 0.9]))  
        pos_cap = float(np.interp(stress, sp, [0.9, 0.022, 0.020, 0.020]))
        deadband = float(np.interp(stress, sp, [0.00, 0.01, 0.02, 0.040]))
